...
subtitle: "Data Processing II: Data Visualization and Synthesis"
toc: true
order: 5
---
## Learning Objectives

In this session, we'll walk through how to effectively visualize data. We'll cover:

- Why data visualization is important
- Strategies for effectively visualizing data
- How to create basic graph types in R

You'll also walk away with some example code to modify and adapt to your own work.

## Plan for the Session

The session will be split into the following sections:

I.   Theory of data visualization
II.  Data visualization practical
...
## I. Theory of data visualization
<iframe class="slide-deck" width="100%" height="800px" src="data_viz_gsa.html"></iframe>
...
Notice that `ggplot2` comes with its own default color scheme. I would discourage you from using the default colors. Instead, we'll try out some of the accessible color palettes we talked about earlier. First, let's try one of the `viridis` color palettes. Since this palette is included in `ggplot2`, all we need to do is add the proper "scale" to our `ggplot()` call.

```{r}
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + 
  geom_point() + #adds the points
  scale_color_viridis_d() #applies the viridis color palette
```
...
Outside of color, there are many other [aspects of the graph](https://ggplot2.tidyverse.org/articles/ggplot2-specs.html) that we can modify using the "scale" functions. For example, we can modify the x and y axes:

```{r}
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + 
  geom_point() +
  scale_color_brewer(name = "Vehicle Class", palette = "Set1") +
  scale_x_continuous(name = "Engine Displacement (l)") + #adding x axis label
  scale_y_continuous(name = "Highway Mileage (mpg)") #adding y axis label
```

#### Theming

The last basic thing you might want to do with `ggplot2` is modify the style of the visualization. This is extremely customizable, but the first place to start is with a [built-in theme](https://ggplot2.tidyverse.org/reference/ggtheme.html). For example, here's what the classic theme looks like:
...
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + 
  ...
  theme_classic() #applies the classic theme
```
...
ggplot(mpg, aes(x = displ, y = hwy, color = class)) + 
  ...
  scale_y_continuous(name = "Highway Mileage (mpg)") +
  theme_classic() +
  theme(axis.text = element_text(color = "black")) #modifies the axis text
```

And there we have it! With just 7 lines of code, we've created what I would say is a publication quality graph! `ggplot` does a lot of the tedious work for you, giving you time to focus on the more important aspects, such as labeling and color. Admittedly, I've spent a LOT of time on these aspects in the past...

For comparison, here's how we would produce a similar graph using only R's built-in (base) plotting functions. It's worth taking a moment to see how ggplot's additive functions all work together to produce a final product, whereas base R requires lots of simultaneous arguments inside a single function, plus sequential additive functions that ggplot accounts for by default (e.g., adding a legend).
...
plot(x = mpg$displ, y = mpg$hwy,
     col = factor(mpg$class), pch = 16,
     xlab = "Engine Displacement (l)",
     ylab = "Highway Mileage (mpg)")
legend("topright", legend = unique(mpg$class), cex = 0.8,
       pch = 16, col = unique(factor(mpg$class)),
       ncol = 2, bty = 'n', title = "Vehicle Class")
```
...
```{r}
ggplot(mpg, aes(x = displ, y = hwy, color = class, shape = drv)) +  #adds a grouping variable (drive type) displayed as different shapes
  geom_point() +
  scale_color_brewer(name = "Vehicle Class", palette = "Set1") +
...
ggplot(mpg, aes(x = displ, y = hwy, color = class, shape = drv)) + 
  ...
  scale_x_continuous(name = "Engine Displacement (l)") +
  scale_y_continuous(name = "Highway Mileage (mpg)") +
  facet_wrap(vars(year)) + #facets by year
  theme_classic() +
  theme(axis.text = element_text(color = "black"))
```
...
ggplot(mpg, aes(x = displ, y = hwy, color = class, shape = drv)) + 
  ...
  scale_x_continuous(name = "Engine Displacement (l)") +
  scale_y_continuous(name = "Highway Mileage (mpg)") +
  facet_grid(rows = vars(cyl), cols = vars(year)) + #facets by number of cylinders and by year
  theme_classic() +
  theme(axis.text = element_text(color = "black"))
```
...
ggplot(mpg, aes(x = hwy)) + 
  geom_histogram() + #creates a histogram of the variable hwy
...
```{r message = FALSE}
ggplot(mpg, aes(x = as.factor(cyl), y = hwy)) + 
  geom_violin(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) + #creates a violin plot
  scale_x_discrete(name = "# Cylinders") +
  scale_y_continuous(name = "Highway Mileage (mpg)") +
  theme_classic() +
  theme(axis.text = element_text(color = "black"))
...
Note that many of these "geoms" have lots of options. For example, we've decided to `scale` all of the violin plots to the same width and to draw the quartiles on them (mimicking the boxplots above).
...
ggplot(faithful, aes(x = eruptions, y = waiting)) +
  geom_density_2d(linewidth = 0.25, colour = "black") + #creates a contour plot
  xlim(0.5, 6) + #adjusting limits of x axis
  ylim(35, 110) + #adjusting limits of y axis
  theme_classic() +
  theme(axis.text = element_text(color = "black"))
```
...
ggplot(faithful, aes(x = eruptions, y = waiting)) +
  ...
  geom_density_2d(linewidth = 0.25, colour = "black") + #this is how we can layer visualizations
  xlim(0.5, 6) +
  ylim(35, 110) +
  theme_classic() +
  theme(axis.text = element_text(color = "black"))
...
::: {.callout-note title="Practice Time!"}
Your turn! Let's see what you and those sitting around you can produce using the iris dataset built into R. Here's a basic plot of Sepal Length versus Petal Length:

```{r}
#look at the data (i.e., what do we have to play with?)
head(iris)
...
Edit the R script directly above by changing the variables, coloring the points by species, fixing the axis labels, etc.
:::
...
Now let's prepare our reef data that comes with the palaeoverse package:
...
ggplot(data = occdf) +
  geom_points_range(aes(x = age, y = accepted_name)) + #plots occurrences
  # geom_point(aes(x = age, y = accepted_name)) +
  # geom_line(aes(x = age, y = accepted_name))
...
### **BREAKOUT SESSION 2**
::: {.callout-note title="Practice Time!"}
Let's now put what you've learned to work to address the question posed toward the beginning of the workshop with the Cenozoic crocodile dataset: *How do paleolatitudinal range and paleodiversity vary for Crocodylia across the Cenozoic?*

1. How might you visualize the crocodile occurrences to look at patterns in range size and/or diversity across time intervals? You could start by sketching on paper what a figure might look like before you even start coding.
2. Try to adapt some of the code shared during the Data Processing modules to make this plot using the cleaned crocodile dataset. For some additional help, you can check out [this Palaeoverse vignette](https://palaeoverse.palaeoverse.org/articles/tetrapod-biodiversity.html).
3. Share your figures in small groups and discuss the different ways in which you chose to display the crocodile data. Moving forward, how might you revise your figure to more effectively visualize the data?
:::
