...
# Follow along?
You can follow along by copy and pasting (or writing) the code along with me in an R markdown or you can copy the entire module into an R markdown file from Github: [Workshop module on Github](https://github.com/palaeoverse/2025-GSA-workshop/tree/main/04_exploration) 
...
library(fossilbrush)
# Load data
fossils <- read.csv("/Users/marknikolic/Downloads/cenozoic_crocs.csv")
```
...
```{r}
# Add formation name
fossils[which(fossils$collection_no == "18539"), "formation"]
...
```{r save_data}
# Save data
write.csv(x = fossils, file = "./cenozoic_crocs_clean.csv", row.names = FALSE)
```
